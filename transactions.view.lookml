- view: transactions
  ...
  - dimension: account_name
    type: string
    sql: |
      CASE WHEN ${account_name_original} = "xxxxxx7630 S07" THEN "Checking Account 7630"
  ...
  - dimension: account_type
    type: string
    sql: |
      CASE WHEN ${account_name} LIKE "Savings%" THEN "Savings"
  ...
  - dimension: amount
    type: number
    sql: |
      CASE WHEN ${transaction_type} = 'debit' THEN -1*${TABLE}.Amount
      ELSE ${TABLE}.Amount
      END
  ...
  - dimension: raw_shit
    type: string
    sql: |
      concat(RIGHT(transactions.Date, 4), '-', CASE WHEN length(SUBSTRING(transactions.Date, 1, length(transactions.Date) - length(substring(transactions.Date,2,8)))) = 1 THEN concat('0',SUBSTRING(transactions.Date, 1, length(transactions.Date) - length(substring(transactions.Date,2,8))))
